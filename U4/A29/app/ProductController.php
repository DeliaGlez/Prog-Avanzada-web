...
class ProductController
{
	...
	public function store($name, $slug, $description, $features, $brand_id, $categories, $tags,$cover) {
        ...
        if (isset($responseData['data'])) {
            header('Location: ' . BASE_PATH . 'products?success=ok');
        } else {
            header('Location: ' . BASE_PATH . 'products?error=error');
        }
    ...
    public function delete($id) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://crud.jonathansoto.mx/api/products/' . intval($id), 
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
